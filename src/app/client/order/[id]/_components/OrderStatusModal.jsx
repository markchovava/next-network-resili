...
import { _orderStatusUpdateAction } from '@/actions/OrderActions';
...
export default function OrderStatusModal({id, getData, domData, isModal, setIsModal}) {
    ...
    async function postData() {
        if(!data?.status){
            const message = "Status is is required"
        ...
        try{
            const res = await _orderStatusUpdateAction(formData, id)
            console.log('res', res)
            if(res?.status == 1) {
                await getData();
