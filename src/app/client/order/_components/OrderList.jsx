...
import { _orderDeleteAction, _orderListAction, _orderListByStatusAction, _orderListByUserAction, _orderListByUserStatusAction, _orderPaginateAction, _orderSearchAction, _orderSearchByUserAction } from '@/actions/OrderActions'
...
export default function OrderList({ dbData }) {
  ...
  const [data, setData] = useState({status:''})
      ...
      async function getSearchData() {
        ...
        try{
        const res = await _orderSearchByUserAction(search);
        orderDispatch({type: 'ADD_DATA', payload: {
            items: res?.data,
            prevURL: res?.links?.prev,
            nextURL: res?.links?.next,
      ...
      async function getDataByStatus(status) {
        ...
        try{
            const res = await _orderListByUserStatusAction(status);
            console.log('res', res)
                orderDispatch({type: 'ADD_DATA', payload: {
                    items: res?.data,
                    prevURL: res?.links?.prev,
      ...
      async function getData() {
          try{
              const res = await _orderListByUserAction();
                  orderDispatch({type: 'ADD_DATA', payload: {
                      items: res?.data,
                      prevURL: res?.links?.prev,
                      nextURL: res?.links?.next,
              ...
              { orderState?.items?.length > 0 ?
                ...
                <div key={key} className='mx-auto w-[100%] py-2 flex items-center justify-start border-b border-x border-gray-300'>
                  ...
                  <div className='w-[10%] px-3 py-2 text-end flex items-center justify-end gap-3 text-xl'>
                      <Link title='View' href={`/client/order/${i?.id}`}> 
