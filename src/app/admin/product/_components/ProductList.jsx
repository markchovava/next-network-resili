...
import { _productDeleteAction, _productListAction, _productPaginateAction, _productSearchAction } from '@/actions/ProductActions'
...
export default function ProductList({ dbData, brandData, }) {
  ...
  return (
    ...
    <section className='w-full pt-[4rem] pb-[5rem]'>
      ...
      <div className='mx-auto w-[92%]'>
        <h3 className='text-[2.5rem] font-light mb-1'>Product List</h3>
      ...
      <section className="mx-auto w-[92%] lg:overflow-hidden overflow-scroll pb-[2rem]">
        <section className='lg:w-[100%] w-[70rem]'>
          ...
          <div className='mx-auto w-[100%] text-lg py-2 flex items-center justify-start font-bold font-white bg-gray-200 '>
            ...
            <div className='w-[20%] border-r border-white px-3 py-2'>PRICE</div>
          ...
          <div key={key} className='mx-auto w-[100%] py-2 flex items-center justify-start border-b border-x border-gray-300'>
            ...
            <div className='w-[20%] border-r border-gray-300 px-3 py-2'>
                { i?.created_at ? formatDate(i?.created_at) : 'Noted Added' }</div>
            <div className='w-[20%] border-r border-gray-300 px-3 py-2'>
                { i?.user?.email ? i?.user?.email : 'Not Added' }</div>
            <div className='w-[10%] px-3 py-2 text-end flex items-center justify-end gap-3 text-xl'>
                <Link title='Edit Category' href={`/admin/product/category/${i?.id}`}> 
                <MdOutlineCategory className='hover:text-green-500 duration-150 hover:scale-110 transition-all ease-in'/> 
                </Link> 
                <Link title='View' href={`/admin/product/${i?.id}`}> 
